#!/bin/sh
# script from https://github.com/guo-yong-zhi/kindle-filebrowser/blob/main/filebrowser_wrapper.sh
col=$((`cat COL.txt`))
...
if [ "$(ps aux | grep '[a]nchordav-linux-arm')" ];then killall anchordav-linux-arm;fi
"$@" &
